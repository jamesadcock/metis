...
export class SingleLayerPerceptron {
  ...
  public train(
    ...
    for (let epoch = 0; epoch < epochs; epoch++) {
      ...
      const gradients = this.computeGradients(featuresWithBias, labels, weights);
  ...
  private computeGradients(features: Matrix, labels: Matrix, weights: Matrix): Matrix {
    ...
    return featuresTransposed.multiplyMatrices(predictionErrors).divide(features.rows);
  }

  // Log the loss at each iteration
  private logLoss(features: Matrix, labels: Matrix, weights: Matrix, iteration: number): void {
