...
describe("loadTraining", () => {
  it(`should return the correct data as a single batch when no batch
    ...
    expect(result.trainingFeatureBatches[0].get()).toStrictEqual([
      [13, 26, 9],
      [2, 14, 6],
    ]);
    expect(result.trainingLabelBatches[0].get()).toStrictEqual([[44], [23]]);
  ...
  it(`should return the correct data with 2 batches when a batch
    ...
    expect(result.trainingFeatureBatches[0].get()).toStrictEqual([[13, 26, 9]]);
    expect(result.trainingFeatureBatches[1].get()).toStrictEqual([[2, 14, 6]]);

    expect(result.trainingLabelBatches[0].get()).toStrictEqual([[44]]);
    expect(result.trainingLabelBatches[1].get()).toStrictEqual([[23]]);
